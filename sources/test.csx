...
class TF
{
    ...
    readonly ConcurrentQueue<(string Key, object)> c_ls = new ConcurrentQueue<(string Key, object)>();
    int ci = 0, _miss = 0, _a = 0, _err = 0;
	
	bool _find_(string ck)
	{
		return c_ls.Any(_ => _.Key == ck);
	}
	
	void _add_((string, object) item)
	{
		c_ls.Enqueue(item);
	}
    ...
    public override string ToString()
    {
		...
		foreach (var (key, v) in c_ls.ToArray())
		{
			sb.AppendLine($"{key}={v};");
		}
        return sb.ToString();
